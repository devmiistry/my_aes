...
int main() {
	...
	// Writing plain-text data to coprocessor memory.
	for(ii = 0; ii<16; ii = ii+4) {
		
		printf("Enter the cipher plain-text data in hexadecimal, to write in location = %x\n", ii);
		mm_reg = ii;
		scanf("%x", &mem_data);
		h2p_lw_coproc_addr_memory=virtual_base + ((unsigned long)(ALT_LWFPGASLVS_OFST + MY_AES128_IP_0_BASE + mm_reg) & \
	...
	for (ii = 0; ii < 16; ii = ii + 4){
		...
		printf("Plain-text data read: %x\n", mem_data);
	}

	for(ii = 0; ii<16; ii = ii+4) {
		
		printf("\n\nEnter the cipher key data in hexadecimal, to write in location = %x\n", ii);
		mm_reg = ii;
		scanf("%x", &mem_data);
		h2p_lw_coproc_addr_memory=virtual_base + ((unsigned long)(ALT_LWFPGASLVS_OFST + MY_AES128_IP_0_BASE + mm_reg) & \
		(unsigned long)(HW_REGS_MASK));
		printf("Writing cipher key data to memory address = %p\n", h2p_lw_coproc_addr_memory);
		*(uint32_t *)h2p_lw_coproc_addr_memory = mem_data;

	}
	
	
	printf("Reading key data locations\n");
	for (ii = 0; ii < 16; ii = ii + 4){
		mm_reg = ii;
		h2p_lw_coproc_addr_memory=virtual_base + ((unsigned long)(ALT_LWFPGASLVS_OFST + MY_AES128_IP_0_BASE + mm_reg) & \
		(unsigned long)(HW_REGS_MASK));
